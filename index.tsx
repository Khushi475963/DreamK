...
11. Dr. Rohit Dhadwal (Urologist) - 10:00AM–2:00PM (Every 1st Wed, ₹400)
12. Dr. Mohit Kaushal (Pulmonologist) - 11:00AM–1:00PM (2nd & 4th Wed, ₹400)
13. Dr. Anil Walia (Cosmetologist) - 10:00AM–2:00PM (Every Sat, ₹100)
14. Dr. Sudhanshu Budakoti (Cardiologist) - 11:00AM–2:00PM (Every 3rd Fri, ₹400)
15. Dr. Deepti Singh (Breast & Endocrine Surgeon) - 10:00AM–2:00PM (Every 2nd Tue, ₹400)
16. Dr. Mahindra Dange (Paediatric Surgeon) - 10:00AM–2:00PM (Every 4th Tue, ₹400)
17. Dr. Nishit Sawal (Neurologist) - 11:00AM–2:00PM (1st & 3rd Tue, ₹500)
18. Dr. Yogesh Jindal (Neuro Surgeon) - 11:00AM–2:00PM (2nd & 4th Thu, ₹400)
19. Dr. Kalpesh (Nephrologist) - 10:00AM–2:00PM (Every 1st Wed, ₹400)
`;
...
const App: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        2. Identify probable conditions (HIGH/MODERATE/LOW).
        3. Recommend specific actions.
        4. Select the MOST APPROPRIATE doctor from the J.C Juneja Hospital list for this patient.
      ...
      alert("Unable to process analysis.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 selection:bg-indigo-100 pb-20">
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-20 px-4 py-4 sm:px-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight">Health Advisor</h1>
              <p className="text-[9px] uppercase font-black text-slate-400 tracking-[0.2em]">Clinical Support</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto mt-10 px-4 sm:px-8">
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
          ...
          <div className="xl:col-span-5 space-y-8">
            <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h2 className="font-bold text-slate-800 text-lg">Patient Profile</h2>
              </div>
            ...
            <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-bold text-slate-800 text-lg">Describe Symptoms</h2>
              </div>
              <textarea rows={6} value={symptoms} onChange={(e) => setSymptoms(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-[1.5rem] px-6 py-5 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-medium text-slate-700 resize-none" placeholder="What symptoms are being experienced?..." />
            </section>

            <button onClick={handleAnalyze} disabled={loading} className={`w-full py-6 rounded-3xl text-lg font-black text-white shadow-xl transition-all flex items-center justify-center gap-4 ${loading ? 'bg-slate-300' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
          ...
          <div className="xl:col-span-7">
            <div className="bg-white rounded-[3rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[800px]">
              <div className="px-10 py-8 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
                <h3 className="font-black text-slate-900 uppercase tracking-tighter text-xl">Clinical Assessment</h3>
              ...
              <div className="flex-1 p-8 sm:p-12 overflow-y-auto space-y-12">
                {result ? (
                  <>
                    ...
                    {/* Hospital Referral Card - NEW SECTION */}
                      ...
                      <div className="bg-white rounded-[1.4rem] overflow-hidden">
                        <div className="bg-indigo-50 px-8 py-5 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <h4 className="text-[11px] font-black uppercase text-indigo-600 tracking-widest">J.C Juneja Hospital Referral</h4>
                          </div>
                          <span className="text-[10px] font-bold text-indigo-400">Official Recommendation</span>
                        </div>
                        <div className="p-8">
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div>
                              <h5 className="text-2xl font-black text-slate-900 mb-1">{result.suggestedDoctor.name}</h5>
                              <p className="text-indigo-600 font-bold uppercase tracking-widest text-[11px] mb-4">{result.suggestedDoctor.department}</p>
                            ...
                            <div className="bg-slate-50 p-6 rounded-2xl flex-1 border border-slate-100">
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Clinical Reason for Referral</p>
                              <p className="text-sm text-slate-600 font-medium italic">"{result.suggestedDoctor.reasonForReferral}"</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-10">
                      <div>
                        ...
                        <p className="text-slate-700 text-lg leading-relaxed font-semibold bg-indigo-50/30 p-6 rounded-2xl border border-indigo-100/50 italic">
                          "{result.impression}"
                        </p>
                      </div>

                      <div>
                        <h4 className="text-[11px] font-black uppercase text-indigo-500 tracking-widest mb-6">Recommended Actions</h4>
                        <div className="grid grid-cols-1 gap-4">
                          {result.recommendedActions.map((action, i) => (
                            <div key={i} className="flex gap-4 p-5 bg-white border border-slate-200 rounded-2xl">
                              <div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center text-xs font-black flex-shrink-0">
                                {i + 1}
                              </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center py-32 opacity-30 grayscale">
                    ...
                    <h3 className="text-xl font-black text-slate-400 tracking-widest uppercase text-center">Ready to Provide Analysis & Physician Recommendation</h3>
                  </div>
                )}
              </div>
              ...
              <div className="p-8 bg-slate-900 text-white border-t border-slate-800">
                <div className="flex gap-6 items-center">
                  ...
                  <div>
                    <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-rose-500 mb-1">Safety & Legal Disclaimer</h5>
                    <p className="text-[10px] text-slate-400 leading-relaxed font-medium">
                      Assessment based on provided physician lists. Not a diagnosis. Seek emergency care for severe symptoms.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <footer className="mt-20 border-t border-slate-200 py-16 text-center">
        <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.4em]">
          &copy; {new Date().getFullYear()} Private Clinical Instance • J.C Juneja Hospital Support
        </p>
      </footer>
    </div>
  );
